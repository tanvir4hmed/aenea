...
2. **Simulation Studio:** filter by location and room, then save named single alerts for the smoke detector and camera, or save both in a scenario. Studio only saves definitions. In **Command Center**, choose the smoke alert from **Trigger Alert / Scenario**, choose a new incident and send. Clicking a device on the map preselects its single alert, or its related scenario when no single exists. Add the camera alert later to that incident, or select both definitions to send a sequential batch. Overlapping device selections are blocked.
