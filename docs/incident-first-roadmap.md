...
2026-09-25 workflow update supersedes the direct-send Studio design below: Studio now saves server-backed, revision-checked single/scenario definitions without triggering events. Command Center combines selected definitions, validates devices and rejects overlap before delivery. An exact browser-persisted batch tracks partial acceptance and retries remaining payloads. The Alexa briefing is in the narrow right rail, with trigger controls below; the map occupies the wider left column and the body fills the available width. Red map indicators are device-specific: current urgent assessment citations, or three distinct recent smoke devices in one room (five-minute window). Repeated signals from one device, different locations and stale signals cannot satisfy that smoke rule. Red is labelled simulated escalation and does not imply siren execution or verified fire size.
