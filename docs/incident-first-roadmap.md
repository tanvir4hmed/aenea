...
2026-09-25 workflow update supersedes the direct-send Studio design below: Studio now saves server-backed, revision-checked single/scenario definitions without triggering events. Command Center combines selected definitions, validates devices and rejects overlap before delivery. Clicking a map device preselects its saved single alert, or its first related scenario when no single exists. An exact browser-persisted batch tracks partial acceptance and retries remaining payloads. The Alexa briefing and trigger controls use a 420–480px right rail; the map occupies the wider left column and the body fills the available width. Red map indicators are device-specific: current urgent assessment citations, or the latest of two or more distinct recent smoke devices in one room (five-minute window). Related devices keep their own amber state. Repeated signals from one device, different locations and stale signals cannot satisfy that smoke rule. Red is labelled simulated escalation and does not imply siren execution or verified fire size.
