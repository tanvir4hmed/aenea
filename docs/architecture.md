...
Phase 6 guided scenario cards use the same ingestion endpoint as manual signals. Household and Handoff views are clients of the existing MCP tools, not another datastore. Copy/print is user-initiated and no responder integration is implied. This phase adds no AWS resources or changes to the selected architecture image.
