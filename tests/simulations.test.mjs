...
import { selectedSignals, alertStates } from '../apps/web/src/simulations.js';
...
test('repeated signals from one device and stale reports do not cause red escalation', () => {
  ...
  const stale = alertStates(catalog, [event('one'), event('two'), event('three', 'three', 301000)], null, now);
...
test('same room names in different locations cannot combine', () => {
  ...
  assert.equal(alertStates(separate, ['one', 'two', 'three'].map(id => event(id)), null, now).get('one').level, 'amber');
});
test('current urgent assessment makes only cited devices red; stale assessment cannot', () => {
  ...
  assert.equal(alertStates(catalog, events, state, now).get('two').level, 'amber');
