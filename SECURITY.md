...
This is an unverified prototype, not a production-security certification. Household partitioning is enforced in application code over a shared table. The deploy role is privileged within project resources; protect its GitHub OIDC trust and demo environment. Use only synthetic household data. See [operations and security boundaries](docs/operations.md) and the [uncleared release checklist](docs/release-checklist.md). Dependency scanning, adversarial hosted testing and final IAM review remain release gates.
